...
public interface JobAdvertisementDao extends JpaRepository<JobAdvertisement, Integer> {
	...
	List<JobAdvertisement> getByAdvertisementStatusAndApprovalStatus(boolean advertisementStatus,
			boolean approvalStatus);

	@Modifying
	@Query("update JobAdvertisement set advertisementStatus=?3 " + "where jobAdvertisementId=?1 "
			+ "and employer.userId=?2 ")
	int updateJobAdvertisementSetJobAdvertisementStatusForEmployer_userId(int jobAdvertisementId, int employerId,
			boolean status);

	@Modifying
	@Query("update JobAdvertisement set approvalStatus=?2 " + "where jobAdvertisementId=?1")
	...
	List<JobAdvertisement> findAllByAdvertisementStatusAndApprovalStatus(boolean advertisementStatus,
			boolean approvalStatus, Pageable pageable);
